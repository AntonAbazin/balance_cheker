...
const db = require('./db');
const { CHAINS, isValidBlockchain } = require('./providers/tokens');
...
app.post('/api/wallets', async (req, res) => {
  const { label, blockchain, address } = req.body || {};
  ...
  const wallet = db.createWallet({
    label: label && label.trim() ? label.trim() : `Кошелёк ${address.slice(0, 8)}…`,
    blockchain,
    address: address.trim(),
  });

  // Первая проверка — база для дальнейшего сравнения, без уведомления в телеграм.
  // Проверяются сразу USDT и нативная монета блокчейна.
